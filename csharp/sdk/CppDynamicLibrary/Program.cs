 ...
 internal static class Program
 {
   private static async Task RunAsync(string[] args)
   {
     var builder = new ConfigurationBuilder().SetBasePath(AppContext.BaseDirectory)
                                             ...
                                             // Usage: --LibraryPath /path/to/lib.so --Partition cppdynamic
                                             .AddCommandLine(args);
     ...
     var taskInfos  = await taskHandle.GetTaskInfosAsync().ConfigureAwait(false);
     logger.Log(LogLevel.Information, "Task submitted: {taskId}", taskInfos.TaskId);
