...
namespace ArmoniK.Samples.DynamicSubmission.Client
{
  internal static class Program
  {
    ...
    /// <param name="n">Unsigned int n to calculate the sum of each number from 0 to n</param>
    /// <param name="threshold">Threshold to subdivide the addition in table of threshold size</param>
    /// <returns>
                                   ...
                                   uint   threshold)
    {
      ...
      {
        throw new ArithmeticException($"The result is {Encoding.ASCII.GetString(result)} but the expected result is {expectedResult} for n = {n}");
      }
