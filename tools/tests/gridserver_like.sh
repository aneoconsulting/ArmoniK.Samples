...
TestDir=${BASEDIR}/../../Samples/GridServerLike/
...
function deploy() {
  cd ${TestDir}
  cp packages/ArmoniK.Samples.GridServer.Services-v1.0.0-700.zip ${HOME}/data
...
function execute() {
  ...
  echo dotnet run -r linux-x64 -f net6.0 -c Debug $@
  dotnet run -r linux-x64 -f net6.0 -c Debug $@
}
