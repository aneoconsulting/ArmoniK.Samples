...
CPIP=$(kubectl get svc nginx -n armonik -o jsonpath="{.status.loadBalancer.ingress[0]."ip"}" || echo "failure")
CPHOST=$(kubectl get svc nginx -n armonik -o jsonpath="{.status.loadBalancer.ingress[0]."hostname"}" || echo "failure")
export CPIP=${CPHOST:-$CPIP}
export CPPort=$(kubectl get svc nginx -n armonik -o custom-columns="PORT:.spec.ports[1].port" --no-headers=true || echo "failure")
