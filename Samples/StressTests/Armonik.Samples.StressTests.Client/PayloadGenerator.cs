...
namespace Armonik.Samples.StressTests.Client
{
  internal class PayloadGenerator
  {
    ...
    private          int     countGenerated_;
    private          long    maxGenerated_;

    private long minGenerated_ = long.MaxValue;
    private long totalGenerated_;

    public PayloadGenerator(long    baseSize,
                            int     variationPercent,
                            string  distribution,
                            ILogger logger)
    {
      baseSize_ = baseSize;
      variationPercent_ = Math.Max(0,
                                   Math.Min(100,
                                            variationPercent));
    ...
    public double[] GeneratePayload()
    {
      ...
      var arraySize = (int)Math.Max(1,
                                    size / 8);
      return Enumerable.Range(0,
                              arraySize)
                       .Select(x => Math.Pow(42.0 * 8 / size,
                                             1        / 3.0))
                       .ToArray();
    }
      ...
      else
      {
        ...
        switch (distribution_)
        {
          ...
          case "normal":
            ...
            randomValue = Math.Max(-variationAmount,
                                   Math.Min(variationAmount,
                                            randomValue));
            break;
          case "exponential":
            var exp = -Math.Log(1.0 - random_.NextDouble());
            var normalized = Math.Min(1.0,
                                      exp / 5.0);
        ...
        size = (long)Math.Max(8,
                              baseSize_ + randomValue);
      }

      minGenerated_ = Math.Min(minGenerated_,
                               size);
      maxGenerated_ = Math.Max(maxGenerated_,
                               size);
