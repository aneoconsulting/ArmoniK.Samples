...
1. Build the Docker image for the worker:
    ```bash
    docker build -t pymonik-gpu-worker .
    ```

2. Deploy ArmoniK locally by following the instructions at [ArmoniK Documentation](https://aneoconsulting.github.io/ArmoniK/). Ensure you create a new partition named "gpumonik" with the worker's image. Following this example:
...
3. Create an environment and add necessary packages:
```bash
uv init
uv add pymonik "jax[cuda12]"
```

## Usage

Run the Client to check if your GPU is detected:

```bash
uv run checkgpu.py
```

If one or multiple Cuda devices appear in the output, it means that you can use your GPU to compute JAX code !

You can try to run the gpu_cpu.py file to run two sessions, the first one is on GPU and the second is on CPU.
