...
def processor(task_handler: TaskHandler) -> Output:
    """
    Processes a task by computing the power of a base number raised to an exponent.
    ...
    # Deserialize the payload to get the base and exponent
    base, exponent = map(int, payload.decode().split(","))
    logger.info(f"Received base={base}, exponent={exponent}")
    ...
    task_handler.send_results({task_handler.expected_results[0]: result_str.encode()})
